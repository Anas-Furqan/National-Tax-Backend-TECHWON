...
exports.createConsultation = async (req, res, next) => {
  try {
    ...
    if (req.file) {
      const { url, path, error } = await uploadToSupabase(
        req.file.buffer,
      ...
      consultationData.noticeFileUrl = url;
      consultationData.noticeFilePublicId = path; // Store path for deletion
    }
...
exports.deleteConsultation = async (req, res, next) => {
  try {
    ...
    if (consultation.noticeFilePublicId) {
      await deleteFromSupabase(consultation.noticeFilePublicId);
    }
